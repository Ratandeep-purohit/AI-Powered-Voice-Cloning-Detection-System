...
class Call(Base, TimestampMixin):
    ...
    initiated_by_user: Mapped["User | None"] = relationship(
        "User", back_populates="calls"
    )
    audio_segments: Mapped[list["AudioSegment"]] = relationship(
        "AudioSegment", back_populates="call", lazy="select"
    )
    voice_analyses: Mapped[list["VoiceAnalysis"]] = relationship(
        "VoiceAnalysis", back_populates="call", lazy="select"
    )
    risk_scores: Mapped[list["RiskScore"]] = relationship(
        "RiskScore", back_populates="call", lazy="select"
    )
    alerts: Mapped[list["Alert"]] = relationship(
        "Alert", back_populates="call", lazy="select"
    )

    # ── Constraints & indexes ──────────────────────────────────────────────
    __table_args__ = (
        CheckConstraint(
            "source_type IN ('UPLOAD','MICROPHONE','STREAM','SIMULATED')",
        ...
        CheckConstraint(
            ...
            name="ck_calls_duration_ms_non_negative",
        ),
